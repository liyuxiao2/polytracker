...
from app.domains.traders.schema import TradeResponse, TraderListItem, TraderProfileResponse, TrendingTrade
...
async def get_trader_profile(address: str, session: AsyncSession = Depends(get_session)):
...
async def get_trader_trades(
    address: str, limit: int = Query(100, le=500), session: AsyncSession = Depends(get_session)
):
...
async def get_trader_open_positions(
    address: str,
    min_unrealized_pnl: float | None = Query(None, description="Filter positions with unrealized P&L >= this value"),
    session: AsyncSession = Depends(get_session),
):

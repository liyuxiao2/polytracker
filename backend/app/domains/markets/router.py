...
async def get_market_watch(
    ...
    sort_by: str = Query(
        "suspicion_score", regex="^(suspicion_score|volatility_score|total_volume|suspicious_trades_count)$"
    ),
...
async def get_market_trades_count(market_id: str, session: AsyncSession = Depends(get_session)):
...
async def add_tracked_market(market: TrackedMarketCreate):
...
async def discover_markets(request: DiscoverMarketsRequest):
...
async def remove_tracked_market(market_id: str, session: AsyncSession = Depends(get_session)):
...
async def get_price_history(
    market_id: str = Query(..., description="Market condition_id"),
    outcome: str | None = Query(None, description="YES or NO"),
    start_time: datetime | None = Query(None),
    end_time: datetime | None = Query(None),
    interval: str | None = Query(None, description="Filter by interval (1m, 5m, 1h, etc.)"),
...
async def backfill_price_history(request: BackfillRequest):
...
async def backfill_trades(
    ...
    market_ids: list[str] | None = Query(None, description="Optional list of market IDs to filter for"),
):
